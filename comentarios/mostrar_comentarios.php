...
?>
    ...
    <form name="form1">
	<select onChange="f_ordenar('<?php echo $id_marca; ?>','<?php echo $id_producto; ?>')" class="form-control" name="ordenar">
	...
	?>
		...
		<div class="main_comentarios">
			...
			?>
            	<div class="eliminar"><a href="comentarios/eliminar_comentario.php?id_cliente=<?php echo $fila7["id_cliente"]; ?>&id_producto=<?php echo $fila7["id_producto"]; ?>&id_marca=<?php echo $fila7["id_marca"]; ?>"><img width="25px" src="comentarios/imagenes/cerrar.png"></a></div>
            <?php
			}
			?>
            <!-- eliminar -->
...
if ($total_paginas > 1) {
		if ($pagina != 1)
			echo '<li><a href="detalleproducto.php?ordenar='.$ordenar.'&id_producto='.$id_producto.'&id_marca='.$id_marca.'&pagina='.($pagina-1).'" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
		for ($i=1;$i<=$total_paginas;$i++) {
			if ($pagina == $i)
				...
				echo ' <li><a href="detalleproducto.php?ordenar='.$ordenar.'&id_producto='.$id_producto.'&id_marca='.$id_marca.'&pagina='.$i.'">'.$i.'</a></li> ';
		}
		if ($pagina != $total_paginas)
			echo '<li><a href="detalleproducto.php?ordenar='.$ordenar.'&id_producto='.$id_producto.'&id_marca='.$id_marca.'&pagina='.($pagina+1).'" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
	}
	echo '</p>';
...
if ($total_paginas > 1) {
		if ($pagina != 1)
			echo '<li><a href="detalleproducto.php?id_producto='.$id_producto.'&id_marca='.$id_marca.'&pagina='.($pagina-1).'" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
		for ($i=1;$i<=$total_paginas;$i++) {
			if ($pagina == $i)
				...
				echo ' <li><a href="detalleproducto.php?id_producto='.$id_producto.'&id_marca='.$id_marca.'&pagina='.$i.'">'.$i.'</a></li> ';
		}
		if ($pagina != $total_paginas)
			echo '<li><a href="detalleproducto.php?id_producto='.$id_producto.'&id_marca='.$id_marca.'&pagina='.($pagina+1).'" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
	}
	echo '</p>';	
